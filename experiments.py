...
def get_train_loader(divide_list, num_classes, data, gppool, y_setting, batch_size):
    train_loader = []
    ...
    if gppool:
        for unpooled_parts in loader:
            pooled_g = GPPool(data, num_classes, divide_list, unpooled_parts, y_setting)
...
def run_experiments(data, num_classes, backbone, divide_method, num_subgraphs, y_setting, gppool,
                    lr, weight_decay, epochs, eval_steps, runs, cuda):
    ...
    if backbone == "GCN":
        model = GCN(data.num_features, 512, num_classes, 2, 0.2).to(device)
    elif backbone == "APPNP":
        model = Net_APPNP(data.num_features, 512, num_classes, 2, 0.2).to(device)
    elif backbone == "SAGE":
        model = SAGE(data.num_features, 512, num_classes, 3, 0.2).to(device)

    train_loader = get_train_loader(divide_list, num_classes, data, gppool, y_setting, int(num_subgraphs / 4))
